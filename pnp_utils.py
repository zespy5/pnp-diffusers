...
def register_attention_content_control_efficient(model, injection_schedule):
    def sa_forward(self):
        to_out = self.to_out
        if type(to_out) is torch.nn.modules.container.ModuleList:
            to_out = self.to_out[0]
...
def register_conv_content_control_efficient(model, injection_schedule):
    def conv_forward(self):
        def forward(input_tensor, temb):
            hidden_states = input_tensor
            ...
            if self.injection_schedule is not None and (self.t in self.injection_schedule or self.t == 1000):
                ...
                # inject unconditional
                hidden_states[2] = hidden_states[0]
                # inject conditional
                hidden_states[3] = hidden_states[0]
...
def register_attention_style_control_efficient(model, injection_schedule):
    def sa_v_forward(self):
        to_out = self.to_out
        if type(to_out) is torch.nn.modules.container.ModuleList:
            to_out = self.to_out[0]
        else:
